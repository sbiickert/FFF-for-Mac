...
//	public func enumerated() -> EnumeratedSequence<[CGRect]> {
//		return cellFrames.enumerated()
//	}
    ...
    private mutating func updateCellFrames(to cellSize: CGSize) {
		...
		origin.y = frame.height - cellSize.height  // Flipping y axis. This code was originally iOS.
        origin.x += offset.dx
        origin.y -= offset.dy  // Flipping y axis. This code was originally iOS.

        if cellCount > 0 {
            for _ in 0..<cellCount {
				let rect = CGRect(origin: origin, size: cellSize)
				cellFrames.append(rect)
				// print("Cell frame \(i) dimensions: \(rect)")
                origin.x += cellSize.width
                if round(origin.x) > round(frame.maxX - cellSize.width) {
                    origin.x = frame.origin.x + offset.dx
                    origin.y -= cellSize.height  // Flipping y axis. This code was originally iOS.
                }
            }
        }
    }
